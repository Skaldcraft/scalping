...
def add_ema(df: pd.DataFrame, period: int = 20, source: str = "close", name: str = None) -> pd.DataFrame:
    """Add an exponential moving average (EMA) column to a DataFrame."""
    out = df.copy()
    col = name or f"ema_{period}"
    out[col] = out[source].ewm(span=period, min_periods=period, adjust=False).mean()
    return out
...
def get_fib_zones(opening_range, reversal_tp_level: float = 0.382) -> FibZones:
    rng = opening_range.high - opening_range.low
    midpoint = opening_range.midpoint
    cheap = opening_range.high - (0.382 * rng)
